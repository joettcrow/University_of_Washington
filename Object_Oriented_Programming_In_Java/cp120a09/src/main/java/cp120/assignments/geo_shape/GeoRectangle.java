...
public class GeoRectangle extends GeoShape{
    ...
    /**
     * Overrides the shape equals for rectangle specifically
     * @param shape the shape to check for equality
     * @param epsilon the tolerance for checking equality between double values as a double
     * @return the truth value for the check as a boolean
     */
    @Override
    public boolean equals( GeoShape shape, double epsilon ){
        ...
        else if (!equalsDouble(getWidth(),((GeoRectangle) shape).getWidth(),epsilon)){
            response = false;
        }
        else if (!equalsDouble(getHeight(),((GeoRectangle) shape).getHeight(),epsilon)){
            response = false;
        }
        else {
